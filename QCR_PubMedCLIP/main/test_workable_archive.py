...
# Train phase
def test(cfg, model, question_model, eval_loader, n_unique_close, device, overridden_dir = None, s_opt=None, s_epoch=0):
    model = model.to(device)  # BAN_model
    question_model = question_model.to(device)
    result_dir = cfg.TEST.RESULT_DIR
    if overridden_dir is not None:
        result_dir = overridden_dir
